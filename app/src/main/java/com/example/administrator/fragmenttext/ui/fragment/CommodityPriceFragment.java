 ...
 * 类描述：
 * 创建人：WangQing
 * 创建时间：2016/1/26 16:08
 * 修改人：WangQing
 * 修改时间：2016/1/26 16:08
 * 修改备注：
 */
public class CommodityPriceFragment extends Fragment{
    ...
    private RecyclerView commodityPriceRecyclerView;
    private SwipeRefreshLayout commodityPriceSwipeRefreshLayout;
    private CommodityPriceAdapter commodityPriceAdapter;
    private Boolean hasNextPage = false;
    private Boolean nextPageIsLoading = false;
    private List<MMMNetWorkModle.MMMNetWorkModleList> netWorkList = new ArrayList<>();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
       view=inflater.inflate(R.layout.commodity_price,container,false);
        initView();
        loadData();
        refreshData();
        return view;
    ...
    private void refreshData() {
        commodityPriceSwipeRefreshLayout.setOnRefreshListener(OnRefreshListener);
        commodityPriceRecyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
    ...
    private void loadMoreData(){
        ...
        if(DeviceUtil.isOnline(getActivity())){
            pageindex++;
            NetWork.getNewNetWork(path, pageindex, new myCallBack<MMMNetWorkModle>() {
                @Override
                public void Success(MMMNetWorkModle mMMNetWorkModle) {
                    nextPageIsLoading=false;
                    if(mMMNetWorkModle.getList().size()<pagesize){
                        hasNextPage=false;
                    }else{
                        hasNextPage=true;
                    }
                    netWorkList.addAll(mMMNetWorkModle.getList());
                    if(mMMNetWorkModle.getList().size()<pagesize){

                    }
                    commodityPriceAdapter.notifyDataSetChanged();
                }
                @Override
                public void Fail(String fail) {
                    nextPageIsLoading=false;
                    ToastUtil.showWarningToast("获取数据失败",getActivity());
                }
            });
        }else{
        ...
        if(DeviceUtil.isOnline(getActivity())){
            ...
            NetWork.getNewNetWork(path, pageindex, new myCallBack<MMMNetWorkModle>() {
                ...
                public void Success(MMMNetWorkModle mMMNetWorkModle) {
                    ...
                    netWorkList.addAll(mMMNetWorkModle.getList());
                    commodityPriceAdapter.notifyDataSetChanged();
                    if(commodityPriceSwipeRefreshLayout.isRefreshing()){
                        commodityPriceSwipeRefreshLayout.setRefreshing(false);
                    }
                }
    ...
    private void initView() {
        commodityPriceRecyclerView=(RecyclerView)view.findViewById(R.id.commodityPriceRecyclerView);
        commodityPriceSwipeRefreshLayout=(SwipeRefreshLayout)view.findViewById(R.id.commodityPriceSwipeRefreshLayout);
        commodityPriceRecyclerView.setLayoutManager(new LinearLayoutManager(getActivity()));
        commodityPriceAdapter=new CommodityPriceAdapter();
        commodityPriceRecyclerView.setAdapter(commodityPriceAdapter);
        SwipeRefreshLayoutUtil.initStyle(commodityPriceSwipeRefreshLayout);
    }

    class CommodityPriceAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
        @Override
        ...
        public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            ...
            if(viewType==TYPE_FOOTER){
                view =LayoutInflater.from(getActivity()).inflate(R.layout.item_list_loading,parent,false);
                return new FootHolder(view);
            }else{
                view=LayoutInflater.from(getActivity()).inflate(R.layout.item_my_network,parent,false);
        ...
        public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
         if(netWorkList.size()==position){
             if(hasNextPage){
                 ((FootHolder)holder).itemLoadingTextView.setText("加载更多......");
             ...
             final MMMNetWorkModle.MMMNetWorkModleList mMMNetWorkModleList=netWorkList.get(position);
             ((ViewHolder)holder).network.setText(mMMNetWorkModleList.getSn());
             ((ViewHolder)holder).time.setText(mMMNetWorkModleList.getT());
             ((ViewHolder)holder).audit.setText(mMMNetWorkModleList.getStn());
             ((ViewHolder)holder).name.setText(mMMNetWorkModleList.getSalesMan());
             Picasso.with(getActivity())
                     .load(image[position])
                     .error(R.drawable.c)
                     .placeholder(R.drawable.b)
                     .into(((ViewHolder)holder).netWorkImageview);
    ...
    class FootHolder extends RecyclerView.ViewHolder{
        ...
        public FootHolder(View itemView) {
            super(itemView);
            rootView = itemView;
            itemCircularProgressBar = (ProgressBar) itemView.findViewById(R.id.itemCircularProgressBar);
            itemLoadingTextView = (TextView) itemView.findViewById(R.id.itemLoadingTextView);
        }
    }
